...
Read-only paragraphs and list items use the browser's default wrapping unless you opt into presentation-oriented wrapping:

```tsx
<MarkdownEditor readOnly readOnlyTextWrap="pretty" value={markdown} />
```

`readOnlyTextWrap` is applied only while `readOnly` is true, so it does not enable optimized paragraph or list wrapping while typing.
...
The primary variables are `--mdx-editor-bg`, `--mdx-editor-border`, `--mdx-editor-text`, `--mdx-editor-muted`, `--mdx-editor-accent`, `--mdx-editor-accent-contrast`, `--mdx-editor-accent-soft`, `--mdx-editor-annotation-bg`, `--mdx-editor-annotation-active-bg`, `--mdx-editor-code-bg`, `--mdx-editor-hover`, `--mdx-editor-selection`, `--mdx-editor-table-border`, `--mdx-editor-table-stripe`, `--mdx-editor-font`, `--mdx-editor-mono-font`, `--mdx-editor-radius`, `--mdx-editor-padding`, `--mdx-editor-font-size`, `--mdx-editor-line-height`, and `--mdx-editor-read-only-text-wrap`.
